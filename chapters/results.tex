...
Figure \ref{fig:loop_av_comp} compares four different runs, all using the same ARSLR scheme but combined with different AV schemes. It is clear that constant AV (no switches, modulation or reconstruction) performs best at maintaining the loops shape, followed by AVSLR and lastly AVSW (AV switches). This is a direct consequence of how well each scheme smooths out the velocity field, specifically sub-resolution shear-noise. 

Constant AV with $\alpha = 1$ is constantly tackling any kind of noise based on the approach velocity $\omega_{ab}$ of two particles (see eq. \eqref{eq:av_Pi}. AVSLR leaves sub-resolution shear noise largely undamped. Meanwhile AVSW reduces $\alpha$ close to $\alpha_{min}$ because switches only activate when there is compression. 
...
\todo[inline]{remove grid lines from lineplots}
