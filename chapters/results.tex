...
For simplicity we label the baseline scheme, using constant $\alpha_B = 0.5$, in our figures as \texttt{a05}. The schemes employing reconstruction as introduced in §\ref{sec:slr} are labelled \texttt{SLR}, \texttt{SLRB} and \texttt{SLRB2} respectively. All AR schemes use the self-switching $v_{sig,B}$ as defined in \eqref{eq:vsigB}. \todo{sentence on resistive heating and reference eq}
...
Figure \ref{fig:sedov} shows the results after $t=0.02$. Compared to Fig. 8 in \cite{WissingShen2020}, the bean-like shape of our blast wave appears a bit narrower with a slightly thicker shock front. The precise internal structure is hard to compare as their Fig 8. is a bit blurry (especially apparent when comparing the pressure panel). We used same per-particle-h and kernel interpolation plotting method as described in the beginning of §\ref{sec:numerical_tests}. But overall we observe similar structure. We also clearly see the expected double-shock in thermal pressure and the outer ring in the other three panels. 

Figure \ref{fig:sedov_metrics} shows that the conservation of total energy and the mean divergence error both improve with increasing resolution. Compared to the reported number of $\langle \varepsilon_{\text{divB}}\rangle=10^{-5}$ in \cite{WissingShen2020}, these results are still one order of magnitude worse. 
...
Figure \ref{fig:ot_linecuts} shows horizontal pressure slices to be compared with Fig. 33 in \cite{Price2018}. We get a qualitatively and quantitatively very similar profile. With increasing resolution, the shock fronts become sharper. 
